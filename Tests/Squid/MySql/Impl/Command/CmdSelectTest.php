...
class CmdSelectTest extends TestCase
{
	...
	public function test_with_sanity()
	{
		$one = $this->select()->columnAsExp('1', 'num');
		$two = $this->select()->column('2');
		
		$numbers = $one->union($two);
		
		$calc = $this->select()
			->from('numbers')
			->column('num')
			->columnAsExp('num * num', 'squared')
			->columnAsExp('num * num * num', 'cubed');
		...
		$result = $this->select()
			...
			->from('calc')
			->columns(['num', 'squared', 'cubed'])
			->columnAsExp('squared + cubed', 'total')
			->queryAll();
		
		self::assertEquals([[1, 1,1,2], [2, 4, 8, 12]], $result);
	}
}

...
class CmdTransactionTest extends TestCase
{
	...
	public function test_commit_CommandExecuted()
	{
		...
		$conn->expects($this->at(0))->method('execute')->willReturn(true);
		$conn->expects($this->at(1))->method('execute')->with('COMMIT')->willReturn(true);
	...
	public function test_rollback_CommandExecuted()
	{
		$conn = $this->mockConnection();
		$conn->expects($this->at(0))->method('execute')->willReturn(true);
		$conn->expects($this->at(1))->method('execute')->with('ROLLBACK');
		$subject = $this->getSubject($conn);
